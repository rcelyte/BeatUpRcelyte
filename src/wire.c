...
bool wire_room_open(struct WireRoomHandle handle, struct GameplayServerConfiguration configuration, ServerCode code) {
	if(handle.block.host != 0 || handle.block.thread != 0)
		return 1;
	return instance_room_open(handle.block.thread, handle.block.group, handle.sub, configuration, code);
}
